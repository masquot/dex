...
dex_wallet_balances AS (
    ...
    WHERE EXISTS (SELECT * FROM mooniswap."MooniSwap_evt_Swapped" swap WHERE swap.evt_block_time > now() - interval '1week' AND balances.wallet_address = swap.contract_address)
),
balances AS (
    SELECT
        wallet_address,
...
rows AS (
    ...
    FROM (
        ...
        SELECT
            ...
            0.5 AS token_pool_percentage -- :todo: :research: is this always 0.5 for mooniswap ?
        FROM balances
...
VALUES ('*/10 * * * *', $$
    SELECT dex.insert_liquidity_mooniswap(
        (SELECT max(day) - interval '1 days' FROM dex.liquidity WHERE project='Mooniswap'),
