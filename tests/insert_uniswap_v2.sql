...
dex_wallet_balances AS (
    ...
    UNION ALL
    -- get the currenly latest rows in `dex.liquidity` as a starting point to avoid expensive recalculations
    SELECT
        pool_address,
        token_address,
        token_amount_raw,
    ...
    INNER JOIN (SELECT MAX(day) AS day FROM dex.liquidity) liq_latest_row ON liq.day = liq_latest_row.day
...
VALUES ('*/10 * * * *', $$ -- :todo:
    SELECT dex.insert_liquidity_uniswap(
        (SELECT max(day) - interval '3 days' FROM dex.liquidity WHERE project='Uniswap'),
